...
void test1()
{
  int arg1 = 35;
  int arg2 = 42;
  int thread_pid = new_thread_create(worker, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test2()
{
  ...
  for(i = 0; i < 2000; i++) {
    global = 1;
    thread_pid = new_thread_create(worker2, 0, 0);
    assert(thread_pid > 0);
...
void test3()
{
  ...
  for (int i = 0; i < num_threads; i++) {
    int thread_pid = new_thread_create(worker3, &arg1, &arg2);
    assert(thread_pid > 0);
  }
...
void test4()
{
  assert(fib(28) == 317811);
   ...
   for (int i = 0; i < num_threads; i++) {
      int thread_pid = new_thread_create(worker4, &arg1, &arg2);
      assert(thread_pid > 0);
   }
...
{
  int arg1 = 42, arg2 = 24;
  int thread_pid = new_thread_create(worker5, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test6()
{
  ...
  for (i = 0; i < num_threads; i++) {
    int thread_pid = new_thread_create(worker6, 0, 0);
    assert(thread_pid > 0);
  }
...
void test7()
{
  int arg1 = 35;
  int arg2 = 42;
  int thread_pid = new_thread_create(worker7, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test8()
{
   ...
   */
   ...
   int thread_pid = new_thread_create(merge_sort, global_arr, &size);
   assert(thread_pid > 0);
...
void test9()
{
  ...
  int i;
  for (i = 0; i < num_threads; i++) {
    int thread_pid = new_thread_create(worker9, 0, 0);
    assert(thread_pid > 0);
  }
...
{
  int arg1 = 42, arg2 = 24;
  int thread_pid = new_thread_create(worker10, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test11()
{
  ...
  size = (uint64)sbrk(0);
  int thread_pid = new_thread_create(worker11, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test12()
{
  ...
  size = (uint64)sbrk(0);
  int thread_pid = new_thread_create(worker12, &arg1, &arg2);
  assert(thread_pid > 0);
...
void test13()
{
  ...
  for (i = 0; i < num_threads; i++) {
    int thread_pid = new_thread_create(worker13, &arg1, &arg2);
    assert(thread_pid > 0);
  }
...
worker7(void *arg1, void *arg2) {
   ...
   assert(global == 1);
   global++;
   int nested_thread_pid = new_thread_create(nested_worker, &arg1_int, &arg2_int);
...
void merge_sort(void *arg1, void *arg2) {
  ...
  int nested_thread_pid_l = new_thread_create(merge_sort, array, &size_left);
  int nested_thread_pid_r = new_thread_create(merge_sort, array_right, &size_right);
...
worker9(void *arg1, void *arg2) {
  ...
  int nested_thread_pid = new_thread_create(nest_worker, 0, 0);
...
worker10(void *arg1, void *arg2) {
   ...
   int nested_thread_pid = new_thread_create(nested_worker2, &tmp1, &tmp2);
   assert(nested_thread_pid > 0);
...
void worker11(void *arg1, void *arg2) {
  ...
  int nested_thread_id = new_thread_create(nest_worker3, 0, 0);
...
worker13(void *arg1, void *arg2) {
  ...
  assert(arg2_int == 42);
  sleep(3);
  int nested_thread_pid = new_thread_create(empty, &arg1_int, &arg2_int);
  int nested_join_pid = thread_join();
  assert(nested_join_pid == nested_thread_pid);
  exit(0);
}
